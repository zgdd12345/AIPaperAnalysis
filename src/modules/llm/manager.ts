...
export class LLMManager {
    ...
    try {
      ...
      providers.forEach((type) => {
        const apiKey = Zotero.Prefs.get(
          `extensions.aipaperanalysis.${type}.apiKey`,
        ) as string;
        if (apiKey) {
          const config: ProviderConfig = {
            type,
            apiKey,
            baseURL: Zotero.Prefs.get(
              `extensions.aipaperanalysis.${type}.baseURL`,
            ) as string | undefined,
            defaultModel: Zotero.Prefs.get(
              `extensions.aipaperanalysis.${type}.model`,
            ) as string | undefined,
            timeout: Zotero.Prefs.get(
              `extensions.aipaperanalysis.${type}.timeout`,
            ) as number | undefined,
            maxRetries: Zotero.Prefs.get(
              `extensions.aipaperanalysis.${type}.maxRetries`,
            ) as number | undefined,
          };
  ...
  addProvider(config: ProviderConfig): void {
    ...
    // 保存到偏好设置
    Zotero.Prefs.set(
      `extensions.aipaperanalysis.${config.type}.apiKey`,
      config.apiKey,
    );
    if (config.baseURL) {
      Zotero.Prefs.set(
        `extensions.aipaperanalysis.${config.type}.baseURL`,
        config.baseURL,
      );
    }
    if (config.defaultModel) {
      Zotero.Prefs.set(
        `extensions.aipaperanalysis.${config.type}.model`,
        config.defaultModel,
      );
    }
   ...
   * 列出可用模型
   */
  async listModels(providerType?: ProviderType): Promise<LLMModel[]> {
    const provider = this.getProvider(providerType);
   ...
   * 验证API密钥
   */
  async validateApiKey(providerType?: ProviderType): Promise<boolean> {
    try {
      const provider = this.getProvider(providerType);
