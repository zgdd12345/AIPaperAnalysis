...
export async function testConnection(window) {
    const { select, apiKeyInput, baseInput, modelInput } = getProviderElements(window);
    const provider = (select?.value ||
        getPref(`${PREF_PREFIX}provider`, "openai"));
    const apiKey = apiKeyInput?.value.trim();
    if (!apiKey) {
        showError(window, getString("prefs-test-error", { args: { error: "Missing API key" } }));
        return;
    }
    try {
        ...
        showError(window, getString("prefs-test-error", { args: { error: formatError(error) } }));
    }
}
export function openPromptManager(window) {
    window.openDialog("chrome://aipaperanalysis/content/prompt-manager.xhtml", "prompt-manager", "chrome,centerscreen,modal,width=800,height=600");
}
function initProviderSection(window) {
    ...
    function loadProviderFields(provider) {
        ...
        const model = getPref(getProviderPref(provider, "model"), provider === "openai" ? "gpt-4" : "");
...
function initAnalysisSection(window) {
    ...
    const label = window.document.getElementById("aipaperanalysis-temperature-value");
    if (scale && label) {
    ...
    const maxTokens = window.document.getElementById("aipaperanalysis-max-tokens");
    if (maxTokens) {
...
function initPromptTable(window) {
    const container = window.document.getElementById("aipaperanalysis-prompts-table");
    if (!container)
        return;
    const table = new ztoolkit.VirtualizedTable(window)
        ...
        .setProp("getRowCount", () => getPromptManager(window).getAllPrompts().length)
...
async function resetPrompts(window) {
    try {
        ...
        showError(window, getString("prefs-reset-error", { args: { error: formatError(error) } }));
    }
}
async function exportPrompts(window) {
    try {
        const filePath = await Zotero.FilePicker.saveFile(getString("prefs-export-prompts"), "aipaperanalysis-prompts.json");
        if (!filePath) {
            return;
        }
        const data = getPromptManager(window).exportPrompts();
        await Zotero.File.putContentsAsync(filePath, data);
        showInfo(window, getString("prefs-export-success", { args: { path: filePath } }));
    }
    catch (error) {
        showError(window, getString("prefs-export-error", { args: { error: formatError(error) } }));
    }
}
async function importPrompts(window) {
    try {
        ...
        const filePath = await Zotero.FilePicker.openFile(getString("prefs-import-prompts"));
        if (!filePath) {
        ...
        showError(window, getString("prefs-import-error", { args: { error: formatError(error) } }));
    }
}
...
function updatePromptButtons(window) {
    const editBtn = window.document.getElementById("aipaperanalysis-prompts-edit");
    const deleteBtn = window.document.getElementById("aipaperanalysis-prompts-delete");
...
function openPromptEditorDialog(window, prompt) {
    const params = { prompt };
    window.openDialog("chrome://aipaperanalysis/content/prompt-editor.xhtml", "prompt-editor", "chrome,modal,centerscreen", params);
    if (!params.result) {
        return null;
    }
    return {
...
function addPrompt(window) {
    ...
    getPromptManager(window)
        ...
        .catch((error) => showError(window, getString("prefs-add-prompt-error", {
...
function editPrompt(window) {
    ...
    getPromptManager(window)
        ...
        .then(() => refreshPromptTable(window))
        .catch((error) => showError(window, getString("prefs-edit-prompt-failed", {
        args: { error: formatError(error) },
    })));
}
function deletePrompt(window) {
    ...
    getPromptManager(window)
        ...
        .then(() => refreshPromptTable(window))
        .catch((error) => showError(window, getString("prefs-delete-prompt-failed", {
        args: { error: formatError(error) },
    })));
}
