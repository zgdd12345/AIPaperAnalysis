...
export class NoteCreator {
     ...
     * 从分析结果创建笔记
     */
    async createNoteFromAnalysis(result) {
        try {
            // 获取父条目
     ...
     * 批量创建笔记
     */
    async createBatchNotes(results) {
        const notes = [];
        for (const result of results) {
            try {
                // 跳过有错误的结果
                if (result.error) {
                    console.warn(`Skipping note creation for failed analysis: ${result.error}`);
                    continue;
                }
                ...
                console.error(`Failed to create note for item ${result.itemId}:`, error);
                // 继续处理其他笔记
            }
        }
        return notes;
    ...
    buildMarkdownContent(result) {
        ...
        if (result.usage) {
            parts.push(`- **Token使用**: ${result.usage.totalTokens} ` +
                `(输入: ${result.usage.promptTokens}, 输出: ${result.usage.completionTokens})\n`);
        }
        parts.push("\n");
        parts.push("---\n");
        parts.push("*此笔记由 [AI Paper Analysis](https://github.com/yourusername/AIPaperAnalysis) 插件自动生成*\n");
        parts.push(this.buildMetadataComment(result));
        return parts.join("");
    }
    buildMetadataComment(result) {
        const metadata = {
     ...
     * 更新现有笔记
     */
    async updateNote(noteId, result) {
        try {
     ...
     * 检查是否已存在相同提示词的分析笔记
     */
    async findExistingNote(itemId, promptId) {
        try {
            ...
            for (const note of notes) {
                ...
                const hasPromptTag = tags.some((tag) => tag.tag === `prompt:${promptId}`);
     ...
     * 创建或更新笔记
     */
    async createOrUpdateNote(result) {
        // 检查是否已存在
        const existingNote = await this.findExistingNote(result.itemId, result.promptId);
        if (existingNote) {
            // 更新现有笔记
     ...
     * 删除指定文献的所有AI分析笔记
     */
    async deleteAllAnalysisNotes(itemId) {
        try {
            const item = await Zotero.Items.getAsync(itemId);
            if (!item)
                return 0;
