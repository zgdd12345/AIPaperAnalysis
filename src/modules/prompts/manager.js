...
export class PromptManager {
    prompts;
    PREF_KEY = 'extensions.aipaperanalysis.prompts';
    constructor() {
        this.prompts = [];
        this.loadPrompts();
    }
    ...
    loadPrompts() {
        try {
            const stored = Zotero.Prefs.get(this.PREF_KEY);
            if (stored) {
                const parsed = JSON.parse(stored);
                // 恢复Date对象
                this.prompts = parsed.map((p) => ({
                    ...p,
     ...
     */
    getCategories() {
        const categories = new Set(this.prompts.map((p) => p.category).filter((c) => c !== undefined));
        return Array.from(categories);
    }
    /**
     * 添加新提示词
     */
    async addPrompt(name, content, options) {
        const prompt = {
            id: this.generateId(),
            name,
            content,
     ...
     * 从JSON导入提示词
     */
    async importPrompts(json, options) {
        try {
    ...
    searchPrompts(query) {
        ...
        return this.prompts.filter((p) => p.name.toLowerCase().includes(lowerQuery) ||
