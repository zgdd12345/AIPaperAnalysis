...
export class ContextMenuManager {
    ...
    updateMenuItems(doc, popup) {
        ...
        if (categories.length > 0) {
            ...
            categories.forEach((category) => {
                const categoryPrompts = this.promptManager.getPromptsByCategory(category);
    ...
    async handleAnalysis(promptId) {
        try {
            ...
            // 确认分析
            const promptName = this.promptManager.getPromptById(promptId)?.name || "未知";
            const confirmed = await this.confirmAnalysis(regularItems.length, promptName);
            if (!confirmed) {
                return;
            }
            // 创建进度窗口
            ...
            const results = await this.analysisEngine.analyzeBatch({
                ...
                promptId,
                onProgress: (progress) => {
                    progressWindow.changeHeadline(`正在分析: ${progress.currentItem} (${progress.current}/${progress.total})`);
                },
            });
            // 创建笔记
            ...
            progressWindow.addDescription(`成功分析 ${results.filter((r) => !r.error).length} 篇文献`);
    ...
    openPromptManager() {
        ...
        if (!win)
            return;
        // TODO: 实现提示词管理对话框
        win.openDialog("chrome://aipaperanalysis/content/prompt-manager.xhtml", "prompt-manager", "chrome,centerscreen,modal,width=800,height=600", {});
    }
    /**
     * 打开设置
     */
