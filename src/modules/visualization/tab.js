...
export class VisualizationTab {
    ...
    async renderCharts(body, setSectionSummary) {
        try {
            const loadingText = getString("item-section-ai-summary-loading");
            this.updateStatus(body, loadingText);
            const data = await this.aggregator.aggregateAnalysisResults(this.getCurrentFilters());
            const totalItems = data.timeline.reduce((sum, entry) => sum + entry.count, 0);
            if (totalItems === 0) {
                this.disposeCharts();
                this.toggleChartVisibility(body, false);
                const emptyText = this.activeYears.size || this.activeTopics.size
    ...
    renderMethod(body, data) {
        ...
        if (data.methods.length === 0) {
            container.textContent = getString("item-section-ai-summary-empty-methods");
            return;
        }
    ...
    renderCitation(body, data) {
        ...
        if (data.citations.nodes.length === 0 ||
            data.citations.links.length === 0) {
            container.textContent = getString("item-section-ai-summary-empty-citations");
            return;
        }
    ...
    renderWordcloud(body, data) {
        ...
        const chart = this.keywordChart.render(container, data.keywords, (keyword) => this.openKeywordSearch(keyword, body));
    ...
    toggleYearFilter(year, body, setSectionSummary) {
        ...
        this.renderCharts(body, setSectionSummary).catch((error) => console.error("Failed to apply year filter:", error));
    }
    toggleTopicFilter(topic, body, setSectionSummary) {
        ...
        this.renderCharts(body, setSectionSummary).catch((error) => console.error("Failed to apply topic filter:", error));
    }
    getCurrentFilters() {
        const filters = {};
    ...
    bindFilterResetButton(body, setSectionSummary) {
        ...
        button.addEventListener("click", () => {
            ...
            this.renderCharts(body, setSectionSummary).catch((error) => console.error("Failed to reset filters:", error));
        });
    }
    updateFilterSummary(body) {
        ...
        const parts = [];
        if (this.activeYears.size) {
            parts.push(getString("item-section-ai-summary-filter-years", {
                args: {
        ...
        if (this.activeTopics.size) {
            parts.push(getString("item-section-ai-summary-filter-topics", {
                args: {
    ...
    async openKeywordSearch(keyword, body) {
        try {
            ...
            if (itemIds?.length && pane?.selectItems) {
                ...
                pane.focus();
                this.updateStatus(body, getString("item-section-ai-summary-keyword-search-success", {
                    args: { keyword, count: itemIds.length },
                }));
                return;
            }
            Zotero.alert(null, getString("item-section-ai-summary-keyword-search-title"), getString("item-section-ai-summary-keyword-search-empty", {
                args: { keyword },
            }));
        }
        catch (error) {
            console.error("Failed to open keyword search:", error);
            Zotero.alert(null, getString("item-section-ai-summary-keyword-search-title"), getString("item-section-ai-summary-keyword-search-error", {
                args: { keyword },
            }));
        }
    }
    ...
    async handleExport(format) {
        try {
            ...
            const filePath = await Zotero.FilePicker.saveFile(pickerTitle, defaultName);
            if (!filePath) {
                return;
            }
            ...
            Zotero.alert(null, getString("item-section-ai-summary-export-success-title"), getString("item-section-ai-summary-export-success", {
                args: { path: filePath },
            }));
        }
        catch (error) {
            console.error("Failed to export AI summary:", error);
            Zotero.alert(null, getString("item-section-ai-summary-export-error-title"), `${getString("item-section-ai-summary-export-error")}: ${error?.message || error}`);
        }
    }
    buildCSV(data) {
        const escape = (value) => {
        ...
        data.citations.nodes.forEach((node) => {
            lines.push(`Citation Node,${escape(node.title)},${escape(node.year || "")}`);
        });
        lines.push("Citations,Links,");
        data.citations.links.forEach((link) => {
            lines.push(`Citation Link,${escape(link.source)}->${escape(link.target)},${escape(link.weight)}`);
        });
        return lines.join("\n");
    }
