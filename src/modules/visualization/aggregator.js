...
export class DataAggregator {
     ...
     * 聚合所有分析笔记
     */
    async aggregateAnalysisResults(filters) {
     ...
     * 构建年份分布
     */
    async buildTimeline(notes, cache) {
        const timelineMap = new Map();
        for (const note of notes) {
            const parentItem = await this.getParentItem(note, cache);
            if (!parentItem)
                continue;
            const yearField = parentItem.getField("date") || "";
     ...
     * 从笔记内容解析主题关键词
     */
    parseTopicsFromContent(content) {
        const topics = [];
        const patterns = [
            /主题[:：]\s*(.+)/g,
            /研究领域[:：]\s*(.+)/g,
     ...
     * 构建引用关系网络
     */
    async buildCitationNetwork(notes, cache) {
        const nodes = [];
        const links = [];
            ...
            relatedItems.forEach((relatedId) => {
                ...
                if (!seenItems.has(relatedId)) {
                    ...
                    if (relatedItem) {
                        const relatedNode = {
                            id: relatedItem.id,
                            title: relatedItem.getField("title") || "Untitled",
                            year: relatedItem.getField("date")?.substring(0, 4) || "",
                        };
    ...
    async applyFilters(notes, filters, cache) {
    ...
    async getParentItem(note, cache) {
    ...
    async getParentYear(note, cache) {
        const parentItem = await this.getParentItem(note, cache);
        if (!parentItem) {
            return "Unknown";
        }
